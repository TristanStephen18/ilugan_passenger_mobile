...
class TicketScreen extends StatefulWidget {
  TicketScreen({super.key, required this.companyId, required this.current, required this.currentlocc, required this.destination, 
  required this.amount, required this.busnum, required this.companyname, required this.distance, required this.type
  });
...
class _TicketScreenState extends State<TicketScreen> {
  ...
  void createpaymentlink() async {
    String? paymentlink = await ApiCalls().createPayMongoPaymentLink(double.parse(widget.amount));
  ...
  void checkpayment() {
  print("clicked");
  _checkstatustimer = Timer.periodic(const Duration(seconds: 5), (timer) async {
...
Future<void> _checkPaymentStatus(Timer timer) async {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: ispaying ? Colors.white : const Color.fromARGB(255, 182, 179, 179),
      appBar: AppBar(
      ...
      body: ispaying ? const Paying() : SingleChildScrollView(
        child: Padding(
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  link != "" ? EButtons(
                    onPressed: () {
                      Navigator.of(context).push(MaterialPageRoute(builder: (_)=>PaymentScreen(link: link.split(" ")[0])));
                      setState(() {
                        ispaying = !ispaying;
                      });
                      checkpayment();
      ...
      child: Column(
        children: [
          ...
          const Image(
            ...
            const Gap(20),
            TextContent(name: "Payment in process...", fcolor: Colors.black, fontsize: 30, fontweight: FontWeight.bold,),
            const Spacer()
        ],
      ),
    );
